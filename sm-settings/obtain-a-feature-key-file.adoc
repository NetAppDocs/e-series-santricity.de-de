...
.Schritte
. Suchen und notieren Sie im System Manager die Seriennummer des Gehäuses. Gehen Sie im linken Bereich zu menu:Support[Suport Center] und scrollen Sie nach unten zu *View top storage array properties*, um die Seriennummer des Gehäuses anzuzeigen.
. Suchen Sie im System Manager die Feature Enable Identifier. Gehen Sie im linken Bereich zu Menü:Settings[System] und scrollen Sie dann zu *Add-ons*. Suchen Sie die *Feature Enable Identifier*. Notieren Sie die Nummer für die Feature Enable Identifier.
