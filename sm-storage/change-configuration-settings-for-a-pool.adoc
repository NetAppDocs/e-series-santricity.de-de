...
. Wählen Sie den Pool aus, den Sie bearbeiten möchten, und klicken Sie dann auf das Menü:Ellipsis-Symbol[Einstellungen bearbeiten].
+
Das Fenster „Pooleinstellungen bearbeiten“ wird angezeigt.

. Passen Sie die Pool-Einstellungen entsprechend an.
+
.Felddetails
[%collapsible]
====
