...
+
So erstellen Sie manuell einen Pool: Wählen Sie *Speicher* und anschließend *Pools und Volumengruppen*. Klicken Sie auf der Registerkarte *Gesamte Kapazität* auf Menü:Create[Pool].
...
+
So fügen Sie einem bestehenden Pool manuell Laufwerke hinzu: Wählen Sie auf der Seite Pools und Volumengruppen den Pool aus und klicken Sie dann auf *Add capacity*.
...
Sie können die Preservation-Kapazität unter *Pools und Volumengruppen* ändern. Wählen Sie den Pool aus, den Sie bearbeiten möchten. Klicken Sie auf das Menü:Ellipsis-Symbol[Einstellungen bearbeiten] und wählen Sie dann die Registerkarte *Einstellungen*.
...
** Die Volume-Gruppe muss eine gerade Anzahl an Laufwerken enthalten. Falls Sie keine gerade Anzahl an Laufwerken haben und noch nicht zugewiesene Laufwerke vorhanden sind, gehen Sie zu *Pools und volume groups*, um weitere Laufwerke zur Volume-Gruppe hinzuzufügen, und wiederholen Sie den Vorgang.
...
Um SSD-Cache-Statistiken anzuzeigen, wählen Sie im linken Bereich menu:Storage[Pools and volume groups]. Wählen Sie den SSD-Cache aus, für den Sie Statistiken anzeigen möchten, und klicken Sie dann auf das menu:ellipsis icon[View statistics]. Die nominalen Statistiken werden auf der Seite „Statistiken anzeigen“ angezeigt.
...
[NOTE]
====
Der Schieberegler „Additional Optimization Capacity“ ist nur für die Speichersysteme EF80, EF50, EF600 und EF300 verfügbar.
...
Resource Provisioning ist eine Funktion, die in den Speicherarrays EF80, EF50, EF300 und EF600 verfügbar ist, welche es ermöglicht, Volumes sofort ohne Hintergrundinitialisierungsprozess zu verwenden.
...
Die Ressourcenbereitstellung ist standardmäßig auf Systemen aktiviert, deren Laufwerke DULBE unterstützen. Sie können diese Standardeinstellung unter *Pools und volume groups* deaktivieren.
