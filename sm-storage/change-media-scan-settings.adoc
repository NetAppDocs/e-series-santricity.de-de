...
. Klicken Sie auf das Menü:Ellipsis-Symbol [Einstellungen für die Medienprüfung des Volumes ändern].
+
Das Fenster „Einstellungen für die Laufwerksmedienprüfung ändern“ wird angezeigt. Alle Volumes des Speichersystems werden in diesem Dialogfeld angezeigt.

. Um den *Medienscan* zu aktivieren, wählen Sie das Kästchen *Medienscan* aus.
+
