...
+
** 2.048 Volumes (EF80, EF600 und E5700 Serien)
** 1.024 Volumes (EF50 und EF300)
...
[NOTE]
====
Vermeiden Sie die Verwendung von Laufwerken mit Data Assurance (DA)-Funktion, um Kapazität zu einem Pool oder einer Volumengruppe hinzuzufügen, die nicht DA-fähig ist. Der Pool oder die Volumengruppe kann die Fähigkeiten des DA-fähigen Laufwerks nicht nutzen. Erwägen Sie in dieser Situation die Verwendung von Laufwerken, die nicht DA-fähig sind.

====
.Schritte
. Wählen Sie im linken Bereich menu:Storage[Pools und volume groups].
. Wählen Sie den Pool oder die Volume-Gruppe aus, zu der Sie Laufwerke hinzufügen möchten, und klicken Sie dann auf das Menü:Ellipsis-Symbol[Kapazität hinzufügen].
+
...
. Unter *Laufwerke zum Hinzufügen von Kapazität auswählen...* wählen Sie ein oder mehrere Laufwerke aus, die Sie dem vorhandenen Pool oder der vorhandenen Volume-Gruppe hinzufügen möchten.
+
...
** Die Verwendung von Laufwerken ohne Data Assurance (DA)-Unterstützung zum Hinzufügen von Kapazität zu einem DA-fähigen Pool oder einer DA-fähigen Volume-Gruppe wird nicht empfohlen. Der Pool oder die Volume-Gruppe verfügt dann nicht mehr über DA-Funktionen, und Sie haben keine Möglichkeit mehr, DA für neu erstellte Volumes innerhalb des Pools oder der Volume-Gruppe zu aktivieren.
** Die Verwendung von Data Assurance (DA)-fähigen Laufwerken zur Kapazitätserweiterung eines Pools oder einer Volumengruppe, die nicht DA-fähig ist, wird nicht empfohlen, da dieser Pool oder diese Volumengruppe die Funktionen des DA-fähigen Laufwerks nicht nutzen kann (die Laufwerksattribute stimmen nicht überein). Erwägen Sie in diesem Fall die Verwendung von Laufwerken, die nicht DA-fähig sind.
...
DULBE-fähig
 a| 
Gibt an, ob das Laufwerk die Option für Deallocated or Unwritten Logical Block Error (DULBE) besitzt. DULBE ist eine Option auf NVMe-Laufwerken, die es dem Speicherarray EF80, EF50, EF300 oder EF600 ermöglicht, ressourcenbereitgestellte Volumes zu unterstützen.
