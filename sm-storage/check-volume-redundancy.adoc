...
NOTE: Die Funktion „Check Volume Redundancy“ ist derzeit nur für Vorgänge in der Benutzeroberfläche des Legacy System Manager verfügbar.

. Wählen Sie im linken Bereich menu:Storage[Pools und volume groups].
. Klicken Sie auf das Menü:Ellipsen-Symbol[Volume-Redundanz prüfen] für den Pool oder die Volume-Gruppe, die Sie überprüfen möchten.
+
