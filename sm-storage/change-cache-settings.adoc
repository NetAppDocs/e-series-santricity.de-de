...
. Klicken Sie auf das Menü:Ellipsis-Symbol [Einstellungen für den Volumencache ändern].
+
Das Fenster „Volume-Cache-Einstellungen ändern“ wird angezeigt. Alle Volumes auf dem Speicher-Array werden in diesem Dialogfeld angezeigt.

. Aktivieren Sie das Kontrollkästchen neben jedem Volume, für das Sie die Cache-Einstellungen ändern möchten, oder aktivieren Sie das Kontrollkästchen in der Tabellenüberschrift, um alle Volumes auszuwählen.
+
[cols="25h,~"]
|===
