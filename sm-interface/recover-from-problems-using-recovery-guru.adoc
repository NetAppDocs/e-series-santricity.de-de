...
.Schritte
. Wählen Sie im linken Bereich *Home* aus.
. Klicken Sie auf das *Glockensymbol* und dann auf den Link mit der Bezeichnung *Recover from _n_ problems* in der Mitte oben im Fenster.
+
...
+
Die Datei wird im Download-Ordner Ihres Browsers mit dem Namen  `recoveryGuru-failureName-arrayName-yyyy-mm-dd-hh-mm-ss.html` gespeichert.
