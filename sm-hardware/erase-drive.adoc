...
====
. Wählen Sie im Kontextmenü *Erase Drives*.
+
...
. Bestätigen Sie den Vorgang durch Eingabe `erase`, und klicken Sie anschließend auf *Laufwerke löschen*.
+
[CAUTION]
====
...
.Ergebnisse
Der Löschvorgang kann mehrere Minuten oder mehrere Stunden dauern. Sie können den Status im Menü:Storage[Operations in Progress] einsehen. Wenn der Löschvorgang abgeschlossen ist, stehen die Laufwerke für die Verwendung in einer anderen Volume-Gruppe oder einem anderen Datenträgerpool oder in einem anderen Speichersystem zur Verfügung.
